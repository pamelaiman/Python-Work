...
# QUESTION 4.1 CREATING TABLES AND ADDING CORRECT DATA
CREATE TABLE movie_info(movie_ID INT, movie_name VARCHAR (60), movie_length TIME, age_rating VARCHAR (30));
CREATE TABLE screens(screen_ID INT, four_k BOOLEAN);
CREATE TABLE showings(showing_ID INT, movie_ID INT,screen_ID INT, start_time TIME, available_seats INT);
...
FROM movie_info INNER JOIN showings
ON movie_info.movie_ID = showings.movie_ID
WHERE showings.start_time > '12:00:00' AND showings.available_seats > 0
ORDER BY showings.start_time;


## QUESTION 4.3 - RETURN THE MOVIE WITH MOST SHOWINGS
SELECT movie_info.movie_name
FROM movie_info INNER JOIN showings
ON movie_info.movie_ID = showings.movie_ID
GROUP BY movie_info.movie_name
HAVING COUNT(showings.movie_ID) = (SELECT COUNT(showings.movie_ID) FROM showings
GROUP BY showings.movie_ID ORDER BY COUNT(showings.movie_ID) DESC 
LIMIT 1);
